...
async def test_cache_miss_extracts_slices_and_caches(db: FakeAsyncDatabase) -> None:
    owner = ObjectId()
...
async def test_cache_hit_skips_extraction(db: FakeAsyncDatabase) -> None:
    owner = ObjectId()
    material_id = await _seed_ready_material(db, owner)
    materials = MaterialRepository(db, owner)  # type: ignore[arg-type]
    sections = SectionRepository(db, owner)  # type: ignore[arg-type]
...
async def test_material_not_ready_raises_conflict(db: FakeAsyncDatabase) -> None:
    owner = ObjectId()
    materials = MaterialRepository(db, owner)  # type: ignore[arg-type]
    material_id = await materials.create(
...
async def test_unknown_node_id_raises_not_found(db: FakeAsyncDatabase) -> None:
    owner = ObjectId()
    material_id = await _seed_ready_material(db, owner)
    materials = MaterialRepository(db, owner)  # type: ignore[arg-type]
    sections = SectionRepository(db, owner)  # type: ignore[arg-type]
